...
export class AuthEffects {
  ...
  authLogin = createEffect(() => {
    return this.actions$.pipe(
      ...
      switchMap((authData: AuthActions.LoginStart) => {
        ...
        return this.http
          .post<AuthResponseData>(
            'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyDfesVFm4PsQD65w--fWwmjsAL-HdI7gZ4',
            {
              email,
              password,
              returnSecureToken: true,
  ...
  authAutoLogin = createEffect(() => {
    return this.actions$.pipe(
      ...
      filter((action): action is AuthActions.AuthenticationSuccess => !!action)
    );
  });
}
